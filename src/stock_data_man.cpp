...
StockDataMan::StockDataMan(/*KLineWall *p_kwall, */ExchangeCalendar *p_exchange_calendar, TSystem::LocalLogger &local_logger, DataBase &data_base)
   ...
   /* , line_0_t_(cst_line_0_t)
    , line_1_t_(cst_line_1_t)*/
{
...
//ps: will clear all old data in related container
T_HisDataItemContainer* StockDataMan::LoadPartStockData(PeriodType period_type, int nmarket, const std::string &stk_code, int start_index, int len, bool is_index)
{
    T_HisDataItemContainer & items_in_container = GetHisDataContainer(period_type, stk_code);
    items_in_container.clear();
    ...
    char error_str[256] = {"\0"};
    bool ret = false; 
    ret = data_base_.GetPartHisKBars(code, is_index, nmarket, ToTypePeriod(period_type), start_index, len, *p_stk_hisdata_item_vector, error_str);
    local_logger_.LogLocal(TSystem::utility::FormatStr("LoadPartStockData-> GetPartHisKBars %d ret:%d %d", period_type, ret, p_stk_hisdata_item_vector->size()));
    ...
    for( unsigned int k = 0; k < p_data_items.size(); ++k )
    {
        auto k_item = std::make_shared<T_KlineDataItem>(p_data_items[k]); 
        k_item->zhibiao_atoms.push_back(std::move(std::make_shared<MomentumZhibiao>()));
        items_in_container.push_back(std::move(k_item));
...
//ps: will clear all old data in related container
T_HisDataItemContainer* StockDataMan::LoadPartStockData(PeriodType period_type, int nmarket, const std::string &stk_code, const T_DateRange &range, bool is_index)
{
    T_HisDataItemContainer & items_in_container = GetHisDataContainer(period_type, stk_code);
    items_in_container.clear();
    ...
    char error_str[256] = {"\0"};
    bool ret = false; 
    ret = data_base_.GetHisKBars(code, is_index, nmarket, ToTypePeriod(period_type), range, *p_stk_hisdata_item_vector, error_str);
    if( !ret )
        local_logger_.LogLocal(TSystem::utility::FormatStr("LoadPartStockData-> GetHisKBars %d ret:%d %d", period_type, ret, p_stk_hisdata_item_vector->size()));
...
// ret: <appended front,appended back> 
std::tuple<int, int> StockDataMan::AppendPartStockData(PeriodType period_type, int nmarket, const std::string &stk_code, const T_DateRange &range, bool is_index)
{
    assert( !stk_code.empty() );
    int count_appended_front = 0;
    int count_appended_back = 0;
    T_HisDataItemContainer & items_in_container = GetHisDataContainer(period_type, stk_code);
    ...
    char error_str[256] = {"\0"};
    bool ret = data_base_.GetHisKBars(code, is_index, nmarket, ToTypePeriod(period_type), range, *p_stk_hisdata_item_vector, error_str);
    if( !ret || p_stk_hisdata_item_vector->empty() ) // fail
    {
        local_logger_.LogLocal(TSystem::utility::FormatStr("LoadPartStockData-> GetHisKBars %d ret:%d %d", period_type, ret, p_stk_hisdata_item_vector->size()));
        delete p_stk_hisdata_item_vector; 
        p_stk_hisdata_item_vector = nullptr;
        return std::make_tuple(count_appended_front, count_appended_back);
    }
    ...
    if( !items_in_container.empty() ) // old data is exists
    {
        ...
        {// if all smaller 
            for( int k = count; k > 0; --k )
            {
                ...
                k_item->zhibiao_atoms.push_back(std::move(std::make_shared<MomentumZhibiao>()));
                items_in_container.push_front(std::move(k_item));
                ++count_appended_front;
            }
            if( count > 0 ) 
            {
                CaculateZhibiao(items_in_container);
                ReCaculateAveLineZhibiao(period_type, stk_code, items_in_container, 0, items_in_container.size()-1); 
            }
        }else if( data_compare(p_data_items[0], items_in_container.back() ) > 0 )
        {// if all bigger 
            for( int k = 0; k < count; ++k )
            { 
                ...
                k_item->zhibiao_atoms.push_back(std::move(std::make_shared<MomentumZhibiao>()));
                items_in_container.push_back(std::move(k_item));
                ReCaculateZhibiaoItem(items_in_container, items_in_container.size()-1);
                ReCaculateAveLineZhibiaoItem(period_type, stk_code, items_in_container, items_in_container.size()-1); 
                ++count_appended_back;
            }
        }else
        {
    ...
    /*for( int k = 0; k < count; ++k )
    {
        ReCaculateZhibiaoItem(items_in_container, count);
    }*/
    // sort T_KlineDateItems by day from small to bigger
    //std::sort(items_in_container.begin(), items_in_container.end(), dompare);
...
void StockDataMan::StoreEndStockDateItem(PeriodType period_type, const std::string &stk_code, const T_StockHisDataItem &item)
{
    auto entry_iter = end_stk_data_items_.find(period_type);
    if( entry_iter == end_stk_data_items_.end() )
        entry_iter = end_stk_data_items_.insert(std::make_pair(period_type, T_CodeMapAutomDataItemContainer())).first;

    auto in_iter = entry_iter->second.find(stk_code);
    if( in_iter == entry_iter->second.end() )
        in_iter = entry_iter->second.insert(std::make_pair(stk_code, T_StockHisDataItem())).first;

    in_iter->second = item;
}

T_StockHisDataItem * StockDataMan::FindEndStockDateItemFromEndContain(PeriodType period_type, const std::string &stk_code)
{
    auto entry_iter = end_stk_data_items_.find(period_type);
    if( entry_iter == end_stk_data_items_.end() )
        return nullptr;
    ...
    if( in_iter == entry_iter->second.end() )
        return nullptr;
    return std::addressof(in_iter->second);
}

int StockDataMan::GetRelKDateTag(PeriodType type_period, int date, int tmp_hhmm)
{
    int tag_date = date;
    int hhmm = GetRelKHhmmTag(ToTypePeriod(type_period), tmp_hhmm);
    if( hhmm == 0 )
        tag_date = exchange_calendar()->NextTradeDate(date, 1);
    return tag_date;
}
...
void StockDataMan::TraverseSetFeatureData(const std::string &stk_code, PeriodType period_type, bool is_index, int r_start_index, unsigned int max_left_len)
{
    ...
    if( items.size() > 0 )
        ReCaculateAveLineZhibiao(period_type, stk_code, items, 0, items.size()-1); 
}

// ret 0: unupdated; 1: updated last item; 2: appended an item
int StockDataMan::UpdateOrAppendLatestItemStockData(PeriodType period_type, int nmarket, const std::string &stk_code, OUT double &quote_price, bool is_index)
{
    assert( !stk_code.empty() );
    int ret = 0;
    T_HisDataItemContainer & items_in_container = GetHisDataContainer(period_type, stk_code);
    
    int cur_date = items_in_container.back()->stk_item.date;
    int cur_hhmm = items_in_container.back()->stk_item.hhmmss;
    ...
    bool result = data_base_.GetNextKBar(stk_code, is_index, nmarket, ToTypePeriod(period_type), cur_date, cur_hhmm, item);
    if( !result )
        return 0;
    quote_price = item.close_price;
    // debug --------
...
int StockDataMan::AppendLatestItemStockData(PeriodType period_type, int nmarket, const std::string &stk_code,  bool is_index)
{
    assert( !stk_code.empty() );
    int ret = 0;
    T_HisDataItemContainer & items_in_container = GetHisDataContainer(period_type, stk_code);
    ...
    if( items_in_container.back()->stk_item.date <= item.date )
    {
        if( items_in_container.back()->stk_item.date == item.date )
        { 
            if( items_in_container.back()->stk_item.hhmmss < item.hhmmss ) // append
            {
                auto k_date_item = std::make_shared<T_KlineDataItem>(item);
                k_date_item->zhibiao_atoms.push_back(std::move(std::make_shared<MomentumZhibiao>()));
                items_in_container.push_back(std::move(k_date_item));
...
// ps: data has sorted
void StockDataMan::ReCaculateZhibiaoItem(T_HisDataItemContainer &data_items_in_container, unsigned int item_index)
{
...
void StockDataMan::AddAveLineT(unsigned int line_t)
{
    std::lock_guard<std::mutex> locker(ave_zhibiao_lines_t_mutex_);
    if( ave_zhibiao_lines_t_.find(line_t) == ave_zhibiao_lines_t_.end() )
        ave_zhibiao_lines_t_.insert(std::make_pair(line_t, true));
}

void StockDataMan::DelAveLineT(unsigned int line_t)
{
    std::lock_guard<std::mutex> locker(ave_zhibiao_lines_t_mutex_);
    if( ave_zhibiao_lines_t_.find(line_t) != ave_zhibiao_lines_t_.end() )
        ave_zhibiao_lines_t_.erase(line_t);
}

void StockDataMan::ReCaculateAveLineZhibiaoItem(PeriodType period_type, const std::string& code, T_HisDataItemContainer &data_items_in_container, unsigned int item_index)
{
    assert(data_items_in_container.size() > item_index);
    
    auto iter = ave_lines_zhibiao_datas_.find(code);
    if( iter == ave_lines_zhibiao_datas_.end() )
        iter = ave_lines_zhibiao_datas_.insert(std::make_pair(code, T_TypeMapIndepentZhibiao())).first;
    auto iter_sub = iter->second.find(period_type);
    if( iter_sub == iter->second.end() )
        iter_sub = iter->second.insert(std::make_pair(period_type, T_CycleTMapIndepentZhibiaoAtom())).first;

    std::lock_guard<std::mutex> locker(ave_zhibiao_lines_t_mutex_);
    for( auto lines_iter = ave_zhibiao_lines_t_.begin(); lines_iter != ave_zhibiao_lines_t_.end(); ++lines_iter )
    {
        auto line_t = lines_iter->first;
        auto iter_grandson = iter_sub->second.find(line_t);
        if( iter_grandson == iter_sub->second.end() )
            iter_grandson = iter_sub->second.insert(std::make_pair(line_t, std::make_shared<AverageLineZhibiao>(line_t))).first;
        IndependentZhibiao *p_ave_line_zhibiao = iter_grandson->second.get();
        p_ave_line_zhibiao->Caculate(data_items_in_container, item_index);
    }
}

void StockDataMan::ReCaculateAveLineZhibiao(PeriodType period_type, const std::string& code, T_HisDataItemContainer &data_items_in_container, unsigned int beg_index, unsigned int end_index)
{
    assert(beg_index <= end_index);
    for( unsigned int i = beg_index; i <= end_index; ++i )
        ReCaculateAveLineZhibiaoItem(period_type, code, data_items_in_container, i);
}

double StockDataMan::GetAveLineZhibiao(PeriodType period_type, const std::string& code, unsigned int line_t, unsigned int index)
{
    auto iter = ave_lines_zhibiao_datas_.find(code);
    if( iter == ave_lines_zhibiao_datas_.end() )
        iter = ave_lines_zhibiao_datas_.insert(std::make_pair(code, T_TypeMapIndepentZhibiao())).first;
    ...
    if( iter_grandson == iter_sub->second.end() )
        return 0.0;
    return iter_grandson->second->GetVal(index);
}
...
void Traverse_GetStuctLines(IN T_HisDataItemContainer & kline_data_items, int r_start_index, int backward_size, OUT T_StructLineContainer &container)
{ 
        ...
        { 
            ...
            if( index != end )
            {
                ...
                if( is_to_add_line )
                {
                    ...
                    container.push_front(std::move(line));
...
void StockDataMan::AppendStructLinesIfNeccary(PeriodType period_type, const std::string &code, const T_StructLineContainer &lines)
{
    if( lines.empty() )
        return;
    T_StructLineContainer &container = GetStructLineContainer(period_type, code);  
    if( container.empty() )
    {
        container.insert(container.end(), lines.cbegin(), lines.cend());
        return;
    }
    while( !container.empty() && lines.front()->beg_index <= container.back()->beg_index )
    {
        container.pop_back();
    }
    container.insert(container.end(), lines.cbegin(), lines.cend());
...
{   
    T_StructLineContainer &container = GetStructLineContainer(period_type, code); 
    Traverse_GetStuctLines(kline_data_items, r_start_index, kline_data_items.size(), container);
    return;
#if 0
...
double EMA_KSpread(T_HisDataItemContainer &kdatas, TypePeriod type_period, int index)
{
    int T = 111;
    switch (type_period)
    {
    case TypePeriod::PERIOD_1M:
    case TypePeriod::PERIOD_5M:  T = 111; break;
    case TypePeriod::PERIOD_30M:
    case TypePeriod::PERIOD_HOUR: T = 52; break;
    case TypePeriod::PERIOD_DAY:
    case TypePeriod::PERIOD_WEEK: T = 14;  break;
    default: break;
    }
    assert(kdatas.size() > 0);
    assert(kdatas.size() > (unsigned int)index ); 
     
    int start_index = MAX_VAL(index - T - 5, 0);
    double Y = KRef(kdatas, start_index, KAttributeType::HIGH) - KRef(kdatas, start_index, KAttributeType::LOW);
    double result = Y; 
    for( int i = start_index + 1; i <= index; ++i )
    { 
        double temp = KRef(kdatas, i, KAttributeType::HIGH) - KRef(kdatas, i, KAttributeType::LOW);
        result = (2 * (temp) + (T - 1) * Y ) / (T + 1);
        Y = temp;
    }
    return result;
}
...
// include index
bool IsPlatformArea(T_HisDataItemContainer &kdatas, TypePeriod type_period, int index, OUT int &area_left)
{
    assert(index >= 0 && index < (int)kdatas.size());

    const int cst_min_t = 12;          // 5m
    const double cst_max_spread = 3.1; // 5m
    int len_walked = 0;
    double min_price = 99999.9;
    double max_price = 0.0;
    for(int i = index; i >= 0; --i )
    {
        double h = KRef(kdatas, i, KAttributeType::HIGH);
        double l = KRef(kdatas, i, KAttributeType::LOW);
        if( h > max_price )
            max_price = h;
        if( l < min_price )
            min_price = l;
        if( fabs(max_price - min_price) > cst_max_spread )
        {
            area_left = len_walked;
            return len_walked >= cst_min_t;
        }
        ++len_walked;
    }
    return false;
