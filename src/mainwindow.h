...
public:
    ...
    CapitalCurve& capital_curve() { return *capital_curve_; }
    ...
    void UpdateStockData(int cur_date, int cur_hhmm);
    void UpdateStockQuote(const T_QuoteData &quote_data);
    ...
    bool is_auto_next_k(){ return is_auto_next_k_; }
    void is_auto_next_k(bool val){is_auto_next_k_ = val;}
    void OnShowAutoForcast(bool val);

    void OnBackTest();
    void PauseBackTest(bool val);
...
private:
    ...
    std::shared_ptr<CapitalCurve> capital_curve_;
    ...
    bool is_auto_next_k_;
    bool is_run_backtest_;
